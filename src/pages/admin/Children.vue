    ...
    <div class="max-900">
      ...
      >
        <template v-slot:top>
          ...
          <q-btn
            ...
            icon="las la-eye"
            :disable="loading"
            :label="inactive ? $t('table.showAlta') : $t('table.showBaixa')"
            @click="toggleIsBaixa"
          />
          <q-btn
            flat
          ...
          <q-btn
            ...
            color="red"
            @click="donarBaixa"
            :label="inactive ? $t('table.donarAlta') : $t('table.donarBaixa')"
          />
          <q-space />
          <q-input
            borderless
        ...
        <template v-slot:body-cell-inactive="props">
          <q-td :props="props">
            <q-badge v-if="props.value" color="red"> BAIXA </q-badge>
          </q-td>
        </template>
      </q-table>
      ...
      <section>
        ...
        <q-btn
          flat
          icon="las la-broom"
          @click="annualIncrement"
          :label="$t('admin.annualIncrement')"
        />
        WIP
      </section>
  ...
  setup() {
    ...
    const getSelectedString = () => {
      return selected.value.length === 0
        ? ""
        : `${selected.value.length} registre${
            selected.value.length > 1 ? "s" : ""
          } seleccionats de ${pagination.value.rowsNumber}`;
    };
    ...
    const orderBy = {};
    orderBy[pagination.value.sortBy] = pagination.value.descending
      ? "desc"
      : "asc";
    const variables = {
      limit,
    ...
    const { result, onResult, onError, loading, fetchMore, refetch } =
      adminService.getChildren(sanitizeVariables);

    onResult(() => {
      children.value = result.value?.children;
      pagination.value.rowsNumber =
        result.value?.children_aggregate?.aggregate?.count;
